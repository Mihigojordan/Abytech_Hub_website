        ...
        <div class="text-left text-sm space-y-2">
          ...
          <div class="bg-emerald-50 border border-emerald-200 rounded-lg p-3 mt-3 space-y-2">
            <div class="flex items-center gap-2 text-emerald-700">
              ...
              <span>An admin account will be created</span>
            </div>
            <div class="flex items-center gap-2 text-emerald-700">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
              <span>Login credentials will be sent to <strong>${application.email}</strong></span>
            </div>
          </div>
        </div>
      `,
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        Swal.fire({
          ...
          text: `${application.fullName} has been accepted. Admin account created and credentials sent.`,
          confirmButtonColor: PRIMARY_COLOR,
        });
      } catch (err) {
        Swal.fire({
