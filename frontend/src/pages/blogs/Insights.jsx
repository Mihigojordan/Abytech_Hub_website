...
import { insightsData } from "../../stores/insightsData";
                                ...
                                {insightsData.trends.items.map((item) => (
                                    <div key={item.id} className="group flex flex-col bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
                                        ...
                                        <div className="p-8 pb-10">
                                            ...
                                            <Link to={`/blog/${item.id}?type=trend`} className="text-[#ff5a00] font-black inline-flex items-center gap-2 group/link">
                                ...
                                {insightsData.events.items.map((item) => (
                                    <div key={item.id} className="group bg-slate-50 p-8 rounded-[2.5rem] flex flex-col md:flex-row gap-8 items-center border border-transparent hover:border-slate-200 transition-all">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <Link to={`/blog/${item.id}?type=event`} className="inline-flex items-center gap-2 text-slate-900 font-black border-b-2 border-slate-900 pb-1 hover:text-[#ff5a00] hover:border-[#ff5a00] transition-all">
                                ...
                                <div className="relative z-10">
                                    ...
                                    <div className="grid md:grid-cols-2 gap-8">
                                        {insightsData.announcements.items.map((item) => (
                                            <Link key={item.id} to={`/blog/${item.id}?type=announcement`} className="flex gap-6 items-start">
                        ...
                        <Element name="jobs" className="scroll-mt-32">
                            ...
                            <div className="space-y-4">
                                {insightsData.jobs.items.map((item) => (
                                    <Link key={item.id} to={`/blog/${item.id}?type=job`} className="p-8 bg-white border border-slate-100 rounded-[2rem] flex flex-col md:flex-row justify-between items-center gap-8 hover:shadow-xl transition-all hover:border-[#ff5a00]/30 cursor-pointer">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-3 mb-2">
                                                <span className="bg-slate-100 text-slate-500 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter">{item.jobType}</span>
                                        ...
                                        <div className="bg-slate-900 text-white px-8 py-4 rounded-xl font-black hover:bg-[#ff5a00] transition-colors whitespace-nowrap">
                                            View Details
                                        </div>
                                    </Link>
                                ))}
