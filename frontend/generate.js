    ...
    try {
        ...
        for (const tile of iconSpecs.windows) {
            if (tile.width && tile.height) {
                // Rectangular tile
                await sharp(sourceImage)
                    .resize(tile.width, tile.height, { fit: 'contain', background: { r: 255, g: 255, b: 255, alpha: 1 } })
                    .png()
                    .toFile(path.join(publicDir, tile.name));
            } else {
                // Square tile
                await sharp(sourceImage)
                    .resize(tile.size, tile.size, { fit: 'contain', background: { r: 255, g: 255, b: 255, alpha: 1 } })
                    .png()
                    .toFile(path.join(publicDir, tile.name));
            }
        ...
        for (const favicon of iconSpecs.favicons) {
            await sharp(sourceImage)
                .resize(favicon.size, favicon.size, { fit: 'contain', background: { r: 255, g: 255, b: 255, alpha: 1 } })
                .png()
